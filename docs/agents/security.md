...
- **DOC-03 (M, proven).** An identifier is a rule of one rule file only. A family of rules belongs to the file that used it first. The sequence is this file, `docs/agents/accessibility.md`, `docs/agents/resilience.md`, and `docs/agents/bundle.md`. A subsequent file puts its letter before the family, such as `ASOT`, `RSOT`, or `BSOT`. Cause: a citation of an identifier that two files hold sends the reader to two rules. Test: `pnpm security:policy` refuses an identifier that two rule files hold.
