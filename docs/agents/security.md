...
- **DOC-02 (M, proven).** A citation of a rule in a tracked file names a rule of this file or of `docs/agents/accessibility.md`. A citation is the word `Rule` and an identifier. Cause: a renumber makes a citation that sends the reader to no rule. Test: `pnpm security:policy` reads each tracked file and refuses a citation that names no rule.
...
- **CODE-05 (M, proven).** The code does not use `any`, `unknown`, `@ts-ignore`, `@ts-expect-error`, or a non-null `!`. Cause: the Google TypeScript style guide. Each one is a hole in the type proof. Test: `eslint` and `tsc --strict`.
